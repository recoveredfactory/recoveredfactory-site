import { PUBLISHER_URL, SITE_URL } from '$lib/config';
...
export function editionSchema(edition: EditionForSchema): unknown[] {
  const url = abs(editionPath(edition.lang, edition.date));
  ...
  const article = {
    ...
    isPartOf: {
      ...
      datePublished: published,
      isPartOf: { ...PERIODICAL, url: abs(homePath(edition.lang)) },
    },
  };
...
export function archiveSchema(
  ...
  const path = ref ? editionPath(lang, ref) : homePath(lang);
  ...
  return [
    {
      ...
      url: abs(path),
      inLanguage: lang,
      isPartOf: { ...PERIODICAL, url: abs(homePath(lang)) },
      publisher: PUBLISHER,
      mainEntity: {
        '@type': 'ItemList',
        ...
        itemListElement: items.map((item, i) => ({
          '@type': 'ListItem',
          position: i + 1,
          url: abs(editionPath(lang, item.date)),
          name: item.title,
        })),
      },
    },
...
function breadcrumbs(lang: Lang, ref?: string) {
  const trail = [{ name: 'Immigration Daybook', url: abs(homePath(lang)) }];

  // An edition sits under its month, so the trail reads month → day and the
  // month page gets linked from every edition that belongs to it.
  if (ref && ref.length === 10) {
    const month = ref.slice(0, 7);
    trail.push({ name: month, url: abs(editionPath(lang, month)) });
    trail.push({ name: ref, url: abs(editionPath(lang, ref)) });
  } else if (ref) {
    trail.push({ name: ref, url: abs(editionPath(lang, ref)) });
  }
